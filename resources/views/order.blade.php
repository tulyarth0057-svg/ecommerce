...
@media (max-width: 576px) {
    ...
    <div class="breadcrumb-area ptb-100 text-center overflow-hidden"
     style="background-image: url('{{ asset('category_banners/shopping-cart-icon-order-confirmation-banner.webp') }}');
                ...
                <div class="order-success-card">
                    ...
                    <div class="order-details-section">
                        <div class="detail-row">
                            ...
                            <span class="order-number">{{ $order->o_order_number }}</span>
                        </div>

                        <div class="detail-row">
                            <span class="detail-label">Order Date:</span>
                            <span class="detail-value">
                                {{ \Carbon\Carbon::parse($order->o_created_at)->format('d M Y, h:i A') }}
                            </span>
                        </div>

                        <div class="detail-row">
                            <span class="detail-label">Payment Method:</span>
                            <span class="payment-badge">{{ $order->o_payment_method }}</span>
                        </div>

                        <div class="detail-row">
                            <span class="detail-label">Payment Status:</span>
                            <span class="payment-badge">{{ ucfirst($order->o_payment_status) }}</span>
                        </div>

                        <div class="detail-row">
                            <span class="detail-label">Total Amount:</span>
                            <span class="total-amount">₹{{ number_format($order->o_total_amount, 2) }}</span>
                        </div>
                    </div>
                    ...
                    <h3 class="order-items-header">Order Items</h3>
                            ...
                            <div class="order-item-card">
                                <h4 class="item-name">{{ $item->o_i_product_name }}</h4>

                                <p class="item-meta">
                                  Size: {{ $item->o_i_size ?? '-' }} |
                                    Qty: {{ $item->o_i_quantity }} |
                                    Product Price: ₹{{ number_format($item->o_i_product_price, 2) }} |
                                    Size Price: ₹{{ number_format($item->o_i_size_price, 2) }}
                                </p>

                                <div class="item-price">
                                    ₹{{ number_format($item->o_i_total_price, 2) }}
                                </div>
                            </div>
                        @empty
                    ...
                    <div class="shipping-section">
                        ...
                        <div class="shipping-address">
                            <strong>{{ $order->o_name }}</strong><br>
                            {{ $order->o_email }} | {{ $order->o_phone }}<br>
                            {{ $order->o_street_address }}<br>
                            {{ $order->o_city }}, {{ $order->o_state }} {{ $order->o_postcode }}
                        </div>
                    </div>
