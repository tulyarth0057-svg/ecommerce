    ...
    <div class="card order-card mb-4">
        ...
        <div class="info-grid">
            <div class="info-item">
                ...
                <div class="info-value">{{ $order->o_order_number ?? 'N/A' }}</div>
            </div>
            <div class="info-item">
                <div class="info-label">
            ...
            <div class="info-item">
                ...
                <div class="info-value">{{ ucfirst($order->o_payment_method ?? 'N/A')  }}</div>
            </div>
            <div class="info-item">
                <div class="info-label">
                ...
                <div class="info-value">{{ ucfirst($order->o_payment_status ?? 'N/A') }}</div>
            </div>
        ...
        ₹{{ number_format($grandTotal, 2) }}
            ...
            <div class="info-item">
                ...
                {{-- <div class="info-value" style="color: {{ strtolower($order->o_order_status) == 'completed' ? '#10b981' : (strtolower($order->o_order_status) == 'cancelled' ? '#ef4444' : '#f59e0b') }}">
                    {{ ucfirst($order->o_order_status ?? 'N/A') }}
                </div> --}}
            </div>
        </div>
    </div>
        ...
        <div class="address-content">
            <div class="address-item">
                ...
                <div class="address-text">
                    <div class="name">{{ $order->o_name ?? 'N/A' }}</div>
                </div>
            </div>
            <div class="address-item">
                <i class="ri-mail-line"></i>
                <div class="address-text">
                    <div class="detail">{{ $order->o_email ?? 'N/A' }}</div>
                </div>
            </div>
            <div class="address-item">
                <i class="ri-phone-line"></i>
                <div class="address-text">
                    <div class="detail">{{ $order->o_phone ?? 'N/A' }}</div>
                </div>
            </div>
            <div class="address-item">
                <i class="ri-home-line"></i>
                <div class="address-text">
                    <div class="detail">{{ $order->o_street_address ?? 'N/A' }}</div>
                    <div class="detail">{{ $order->o_city ?? 'N/A' }}, {{ $order->o_state ?? 'N/A' }} - {{ $order->o_postcode ?? 'N/A' }}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-md-5">
            <div class="product-image-wrapper">
                ...
                {{-- <span class="status-badge 
                    {{ strtolower($order->o_order_status) == 'completed' ? 'status-completed' : 
            ...
            <div class="card-body">
                ...
                <div class="product-info">
                    <span class="info-badge">
                        ...
                        Quantity: {{ $item->o_i_quantity }}
                    </span>

                    <span class="info-badge">
                        <i class="ri-ruler-line"></i>
                        Size: {{ $item->o_i_size ?? 'N/A' }}
                    </span>
               ...
               <div class="price-summary mt-4">
                        ...
                        <div class="price-row">
                            <span>Shipping</span>
                            {{-- <span>
                                ₹{{ number_format($order->o_shipping_cost ?? 'N/A' / max(count($orderItems),1), 2) }}
                            </span> --}}
                        </div>
                    ...
                    {{-- <button class="payment-status-btn 
                        {{ strtolower($order->o_payment_status ?? 'N/A') == 'paid' ? 'payment-paid' : 'payment-pending' }}">
                        <i class="ri-secure-payment-line"></i>
                        Payment {{ ucfirst($order->o_payment_status ?? 'N/A') }}
                    </button> --}}
                    ...
                    <div class="action-buttons">
                        ...
                        {{-- <a href="{{ route('order.track', ['order_number' => $order->o_order_number]) }}" class="btn btn-orange-outline flex-fill">
